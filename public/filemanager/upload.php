...
Errors::debugLogger(sprintf("StoreFolder: %s StoreFolderThumb: %s", $storeFolder, $storeFolderThumb));
...
if ($path_pos !== 0
    ...
    || strpos($storeFolder, './', strlen($current_path)) !== FALSE) die('wrong path');
...
if (!empty($_FILES))
{
    ...
    if (in_array(fix_strtolower($info['extension']), $ext))
    {
        ...
        Errors::debugLogger(sprintf("TargetFile: %s TargetFileThumb: %s", $targetFile, $targetFileThumb));
        ...
        if ($is_img)
        {
            ...
            else
            {
                if (!new_thumbnails_creation($targetPath, $targetFile, $_FILES['file']['name'], $current_path,
                                             $relative_image_creation, $relative_path_from_current_pos,
                                             $relative_image_creation_name_to_prepend, $relative_image_creation_name_to_append,
                                             $relative_image_creation_width, $relative_image_creation_height, $fixed_image_creation,
                                             $fixed_path_from_filemanager, $fixed_image_creation_name_to_prepend,
                                             $fixed_image_creation_to_append, $fixed_image_creation_width, $fixed_image_creation_height))
                {
                    $memory_error = false;
                }
                else
                {
                    ...
                    if ($resize) create_img_gd($targetFile, $targetFile, $srcWidth, $srcHeight);
                }
            }
            if ($memory_error)
            {
                Errors::debugLogger(__FILE__."@".__LINE__." Memory error!");
                //error
                unlink($targetFile);
    ...
    {
        Errors::debugLogger(__FILE__."@".__LINE__." File not permitted!");
...
{
    Errors::debugLogger(__FILE__."@".__LINE__." Bad request!");
