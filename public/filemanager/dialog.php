...
{
    die("Access Denied to read files");
}
...
if (isset($_POST['submit']))
{
    ...
    if (
        #!ACL::IsUserAuthorized("files", "create") # Uncomment when Live
        ACL::IsUserAuthorized("files", "create") # Uncomment to Unit Test (fail when should pass)
        || #!ACL::IsUserAuthorized("files", "update")) # Uncomment when Live
        ACL::IsUserAuthorized("files", "update")) # Uncomment to Unit Test (fail when should pass)
    {
        die("Access Denied to create/update files");
    }
    include("upload.php");
}
else
{
        ...
        && strpos($_GET['fldr'], './') === FALSE)
    {
        ...
        Errors::debugLogger("* New folder chosen! FLDR => subdir = " . $subdir);
    }
    else
    {
        // First/root visit
        Errors::debugLogger("First visit? Set subdir = ''", 10);
        $subdir = '';
    }

    // Subdir empty (first visit)?
    if ($subdir == "")
    {
        Errors::debugLogger("Subdir is blank...", 10);
        // Restore from last_position?
        if (!empty($_SESSION['RF']['last_position'])
            && strpos($_SESSION['RF']['last_position'], '.') === FALSE)
        {
            $subdir = trim($_SESSION['RF']['last_position']);
            Errors::debugLogger("* Subdir empty but last_position not, restoring subdir to last_position: ".$subdir);
        }
    }

    if ($subdir == "/")
    {
        Errors::debugLogger("Subdir was '/' so setting to ''", 10);
        $subdir = "";
    }
        
    Errors::debugLogger(__LINE__.' FileManager :: SUBDIR = ' . $subdir);

    // Last_Position is saved for browser close, come back to same folder (if subdir blank but this is set, subdir is set to this)
    Errors::debugLogger("Saving subdir as RF last_position to session...", 10);
    $_SESSION['RF']['last_position'] = $subdir;
    
    /* * *
     * SUB-DIR CODE
     * * */
    // rfm_subfolder is prefixed to new subdir as in lastclick.newclick
    $rfm_subfolder = '';
    ...
    {
        Errors::debugLogger("Administrator for main brand is logged on, clearing rfm_subfolder restrictions...", 10);
        $rfm_subfolder = '';
    }
    ...
    {
        Errors::debugLogger("FileManager :: ACL Starting Folder: Public, setting rfm_subfolder = 'Public'...");
        // subdir must begin with Public/ including trailing slash
        if (!preg_match('/^Public\//', $rfm_subfolder))
        {
            $rfm_subfolder = "Public/".$rfm_subfolder;
            Errors::debugLogger("New ACL rfm_subfolder = ".$rfm_subfolder);
            // Creates folder for first use later...
            $forcedACL = TRUE;
        }
    ...
    {
        Errors::debugLogger("Adding trailing slash to rfm_subfolder...",10);
        $rfm_subfolder.="/";
    }
    
    Errors::debugLogger("Checking if path exists: ".$current_path.$rfm_subfolder.$subdir, 10);
    if (!file_exists($current_path.$rfm_subfolder.$subdir))
    {
        // If this doesnt exist because we are restricting root path for store owners etc., create folder for first time:
        if ($forcedACL)
        {
            Errors::debugLogger('ForcedACL is True and folder missing, creating for first time...');
            Errors::debugLogger("Path: ".$current_path.$rfm_subfolder.$subdir);
            Errors::debugLogger("ThumbsPath: ".$thumbs_base_path.$rfm_subfolder.$subdir);
            create_folder($current_path.$rfm_subfolder.$subdir, $thumbs_base_path.$rfm_subfolder.$subdir);
        } else {
            $subdir = '';
            Errors::debugLogger("(Nope, cleared subdir) Checking if path exists: ".$current_path.$rfm_subfolder.$subdir, 10);
            if (!file_exists($current_path.$rfm_subfolder.$subdir))
            {
                Errors::debugLogger("(Nope, cleared rfm_subfolder)", 10);
                $rfm_subfolder = '';
            }
        }
    }
    ...
    {
        Errors::debugLogger("rfm_subfolder is empty, using subdir",10);
    ...
    {
        Errors::debugLogger("rfm_subfolder is not empty, including",10);
    ...
    Errors::debugLogger("Saving RF subfolder ($subdir) to session...", 10);
    $_SESSION['RF']['subfolder'] = $subdir;
    
    Errors::debugLogger(__LINE__." ***********************************",10);
    Errors::debugLogger(__LINE__." FINAL: ",10);
    Errors::debugLogger(__LINE__." current_path: ".$current_path,10);
    Errors::debugLogger(__LINE__." cur_dir: ".$cur_dir,10);
    Errors::debugLogger(__LINE__." cur_path: ".$cur_path,10);
    Errors::debugLogger(__LINE__." thumbs_path: ".$thumbs_path,10);
    Errors::debugLogger(__LINE__." parent: ".$parent,10);
    Errors::debugLogger(__LINE__." rfm_subfolder: ".$rfm_subfolder,10);
    Errors::debugLogger(__LINE__." subdir: ".$subdir,10);
    Errors::debugLogger(__LINE__." scandir: ".$current_path . $rfm_subfolder . $subdir,10);
    Errors::debugLogger(__LINE__." ***********************************",10);
    
    // Ensure thumbs folder exists
    Errors::debugLogger("Checking if thumbs_path.subidr exists: ".$thumbs_path.$subdir,10);
    if (!is_dir($thumbs_path.$subdir))
    {
        Errors::debugLogger("Creating thumbs_path.subdir: ".$thumbs_path.$subdir);
        create_folder(false, $thumbs_path.$subdir);
    } else {
        Errors::debugLogger("...Exists",10);
    }
    
    // Ensure source folder exists
    Errors::debugLogger("Checking if cur_path exists: ".$cur_path,10);
    if (!is_dir($cur_path))
    {
        Errors::debugLogger("Creating cur_path: ".$cur_path);
        create_folder($cur_path, false);
    } else {
        Errors::debugLogger("...Exists",10);
    }
...
// add lang file to session for easy include
    ...
    <html xmlns="https://www.w3.org/1999/xhtml">
        <head>
            ...
            <script>
                var ext_img = new Array('<?php
        echo implode("','", $ext_img)
        ?>');
        ...
        <div class="container-fluid">
            ...
            Errors::debugLogger("Scanning files...",10);
                            ...
                            <div class="filters">
                                <div class="row-fluid">
                                    <div class="span3 half">
                                        ...
                                        <?php } ?>
                                        <?php
                                        if ($copy_cut_files || $copy_cut_dirs)
                                        {
                                            ?>
                                    ...
                                    <div class="span3 half view-controller">
                                        ...
                                        if ($view == 0) echo " btn-inverse";
                                        ?>" id="view0" data-value="0" title="<?php echo lang_View_boxes; ?>"><i class="icon-th <?php
                                                if ($view == 0) echo "icon-white";
                                                ?>"></i></button>
                                        <button class="btn tip<?php
                                        if ($view == 1) echo " btn-inverse";
                                        ?>" id="view1" data-value="1" title="<?php echo lang_View_list; ?>"><i class="icon-align-justify <?php
                                                if ($view == 1) echo "icon-white";
                                                ?>"></i></button>
                                        <button class="btn tip<?php
                                        if ($view == 2) echo " btn-inverse";
                                        ?>" id="view2" data-value="2" title="<?php echo lang_View_columns_list; ?>"><i class="icon-fire <?php
                                                if ($view == 2) echo "icon-white";
                                                ?>"></i></button>
                                    </div>
                                    <div class="span6 half types">
                                        ...
                                        ?>" id="filter-input" name="filter" placeholder="<?php echo fix_strtolower(lang_Text_filter); ?>..." value="<?php echo $filter; ?>"/><?php
                                               if ($n_files > $file_number_limit_js)
                                               {
                                                   ?><label id="filter" class="btn"><i class="icon-play"></i></label><?php } ?>

                                        <input id="select-type-all" name="radio-sort" type="radio" data-item="ff-item-type-all" class="hide"  />
                                        <label id="ff-item-type-all" title="<?php echo lang_All; ?>" <?php
                                        if ($_GET['type'] == 1 || $_GET['type'] == 3)
                                        {
                                            ?>style="visibility: hidden;" <?php } ?> data-item="ff-item-type-all" for="select-type-all" style="margin-rigth:0px;" class="tip btn btn-inverse ff-label-type-all"><i class="icon-align-justify icon-white"></i></label>
                ...
                <div class="span12">
                    ...
                    else
                    {
                        ...
                        <ul class="grid cs-style-2 <?php echo "list-view" . $view; ?>" id="main-item-container">
                            <?php
                            $jplayer_ext = array("mp4", "flv", "webmv", "webma", "webm", "m4a", "m4v", "ogv", "oga", "mp3", "midi",
                                "mid",
                                "ogg", "wav");
                            foreach ($files as $file_array)
                            {
                                $file     = $file_array['file'];
                                if ($file == '.' || (isset($file_array['extension']) && $file_array['extension'] != lang_Type_dir) || ($file ==
                                    ...
                                    ?>">
                                        ...
                                        else
                                        {
                                            ...
                                            <figcaption>
                                                ...
                                                if ($rename_folders && !$file_prevent_rename) echo "rename-folder";
                                                ?>" title="<?php echo lang_Rename ?>" data-path="<?php echo $rfm_subfolder . $subdir . $file; ?>" data-thumb="<?php echo $thumbs_path . $subdir . $file; ?>">
                                                    ...
                                                    if (!$rename_folders || $file_prevent_rename) echo 'icon-white';
                                                    ?>"></i></a>
                                                <a href="javascript:void('')" class="tip-left erase-button <?php
                                                if ($delete_folders && !$file_prevent_delete) echo "delete-folder";
                                                ?>" title="<?php echo lang_Erase ?>" data-confirm="<?php echo lang_Confirm_Folder_del; ?>" data-path="<?php echo $rfm_subfolder . $subdir . $file; ?>"  data-thumb="<?php echo $thumbs_path . $subdir . $file; ?>">
                                                    ...
                                                    if (!$delete_folders || $file_prevent_delete) echo 'icon-white';
                                                    ?>"></i>
                                                </a>
                                            </figcaption>
                                        <?php } ?>
                            ...
                            foreach ($files as $nu => $file_array)
                            {
                                ...
                                if ($file == '.' || $file == '..' || is_dir($current_path . $rfm_subfolder . $subdir . $file) || in_array($file,
                                                                                                                                          $hidden_files) ||
                                    !in_array(fix_strtolower($file_array['extension']), $ext) || ($filter != '' && strpos($file,
                                                                                                                          $filter) === false))
                                        continue;
                                ...
                                if (in_array($extension_lower, $ext_img))
                                {
                                    ...
                                    if (!file_exists($src_thumb))
                                    {
                                        try
                                        {
                                            ...
                                            new_thumbnails_creation($current_path . $rfm_subfolder . $subdir, $file_path, $file,
                                                                    $current_path, $relative_image_creation,
                                                                    $relative_path_from_current_pos,
                                                                    $relative_image_creation_name_to_prepend,
                                                                    $relative_image_creation_name_to_append,
                                                                    $relative_image_creation_width, $relative_image_creation_height,
                                                                    $fixed_image_creation, $fixed_path_from_filemanager,
                                                                    $fixed_image_creation_name_to_prepend,
                                                                    $fixed_image_creation_to_append, $fixed_image_creation_width,
                                            ...
                                            <a href="javascript:void('')" class="link" data-file="<?php echo $file; ?>" data-field_id="<?php echo $field_id; ?>" data-function="<?php echo $apply; ?>">
                                                <div class="img-precontainer">
                                                    ...
                                                    <div class="img-container">
                                                        <span></span>
                                                        <img alt="<?php echo $filename . " thumbnails"; ?>" class="<?php
                                                        echo $show_original ? "original" : ""
                                                        ?> <?php echo $is_icon_thumb ? "icon" : "" ?>" src="<?php echo $src_thumb; ?>">
                                                    </div>
                                                </div>
                                                <div class="img-precontainer-mini <?php if ($is_img) echo 'original-thumb' ?>">
                                                    ...
                                                    <div class="img-container-mini">
                                                        ...
                                                        if ($mini_src != "")
                                                        {
                                                            ?>
                                                            <img alt="<?php echo $filename . " thumbnails"; ?>" class="<?php
                                                            echo $show_original_mini ? "original" : ""
                                                            ?> <?php echo $is_icon_thumb_mini ? "icon" : "" ?>" src="<?php echo $mini_src; ?>">
                                                             <?php } ?>
                                                    </div>
                                                </div>
                                                <?php
                                            ...
                                            <figcaption>
                                                <form action="force_download.php" method="post" class="download-form" id="form<?php echo $nu; ?>">
                                                    ...
                                                    elseif (($is_video || $is_audio) && in_array($extension_lower, $jplayer_ext))
                                                    {
                                                        ...
                                                        ?>"
                                                           title="<?php echo lang_Preview ?>" data-url="ajax_calls.php?action=media_preview&title=<?php echo $filename; ?>&file=<?php
                                                    ...
                                                    <?php } ?>
                                                    <a href="javascript:void('')" class="tip-left edit-button rename-file-paths <?php
                                                    if ($rename_files && !$file_prevent_rename) echo "rename-file";
                                                    ?>" title="<?php echo lang_Rename ?>" data-path="<?php echo $rfm_subfolder . $subdir . $file; ?>" data-thumb="<?php echo $thumbs_path . $subdir . $file; ?>">
                                                        <i class="icon-pencil <?php
                                                        if (!$rename_files || $file_prevent_rename) echo 'icon-white';
                                                        ?>"></i></a>

                                                    <a href="javascript:void('')" class="tip-left erase-button <?php
                                                    if ($delete_files && !$file_prevent_delete) echo "delete-file";
                                                    ?>" title="<?php echo lang_Erase ?>" data-confirm="<?php echo lang_Confirm_del; ?>" data-path="<?php echo $rfm_subfolder . $subdir . $file; ?>" data-thumb="<?php echo $thumbs_path . $subdir . $file; ?>">
                                                        <i class="icon-trash <?php
                                                        if (!$delete_files || $file_prevent_delete) echo 'icon-white';
                                                        ?>"></i>
                                                    </a>
                                                </form>
                                            </figcaption>
