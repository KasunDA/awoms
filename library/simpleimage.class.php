...
class SimpleImage
{
	...
	public function load($filename)
    {
        ...
        $this->original_info = array(
            ...
            'exif'        => function_exists('exif_read_data') && $info['mime'] == 'image/jpeg' ? $this->exif   = @exif_read_data($this->filename)
                    : null, // Fix for GIF
	...
	public function save($filename = null, $quality = null)
    {
        ...
        switch (strtolower($format))
        {
            ...
            case 'jpg':
            case 'jpeg':
                if ($quality === null) $quality = 100; // Changed default from 85 to 100
	...
	public function colorize($color, $opacity)
    {
        ...
        imagefilter($this->image, IMG_FILTER_COLORIZE, $this->keep_within($rgb['r'], 0, 255), $this->keep_within($rgb['g'], 0, 255),
                                                                                                                 $this->keep_within($rgb['b'],
                                                                                                                                    0,
                                                                                                                                    255),
                                                                                                                                    $alpha);
        return $this;
    ...
    private function imagecopymerge_alpha($dst_im, $src_im, $dst_x, $dst_y, $src_x, $src_y, $src_w, $src_h, $pct)
    {
        ...
        for ($x = 0; $x < $w; $x++)
        {
            for ($y = 0; $y < $h; $y++)
            {
                ...
                $alphacolorxy = imagecolorallocatealpha($src_im, ($colorxy >> 16) & 0xFF, ($colorxy >> 8) & 0xFF, $colorxy & 0xFF,
                                                        $alpha);
